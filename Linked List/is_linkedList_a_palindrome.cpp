...
class LinkedList{
    ...
    public:
        ...
        bool isPalindromeUtil(Node *right)
        {
           if(right == nullptr){
               return true;
           }
           
           bool res = isPalindromeUtil(right->next);
           
           // NOTE: If any of the recursive calls have returned false, directly return false
           if(res == false){
               return false;
           }
           ...
           // left->data == right->data. Thus move left and right ahead
           else{
               // move left pointer in HEAP to the next node
               left = left->next;
               // return true for the current recursive call
               return true;
           }
        }
        
        // NOTE NOTE: We are creating left pointer in HEAP
        Node *left;
