...
class LinkedList{
    ...
    public:
        ...
        void foldListUtil(Node *right, int stopper)
        {
            if(right == nullptr){
                return;
            }
            
            foldListUtil(right->next, stopper + 1);
            
            // keep adjusting pointers as per intuition until rightPtr is ahead of mid
            if(stopper > size / 2){
                Node *temp = left->next;
                left->next = right;
                right->next = temp;
                // move left ahead for next folding recursion call
                left = temp;
            }
            
            // when rightPtr reaches mid-node; this mid-node is new tail.
            else if(stopper == size / 2){
                tail = right;
                right->next = nullptr;
            }
            
            else{
                // Do nothing when rightPtr crosses mid-node on it's way back from
                // recursive calls.
            }
        ...
        void foldList()
        {
            ...
            // NOTE NOTE: Now, we want to STOP adjusting pointers when 
            // rightPtr reaches mid of the linked list. Thus, pass another variable
            // in recursive call to make that happen.
...
int main()
{
    ...
    // NOTE: We want to traverse list with two pointers; one from left to right
    //another from right to left, in O(n). So, keep leftPtr in HEAP and rightPtr
    // in RECURSION STACK.
