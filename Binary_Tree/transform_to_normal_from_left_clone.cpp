...
Node* leftCloneToNormal(Node *root)
{
    ...
    // recursively normalise the root->left->left and right subtree
    Node* leftNormalRoot = leftCloneToNormal(root->left->left);
    Node* rightNormalRoot = leftCloneToNormal(root->right);
    
    // make the leftNormalRoot as the leftchild of current node.
    // This will break the left link to the clone node
    root->left = leftNormalRoot;
